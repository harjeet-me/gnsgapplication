...
export class PRoulComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    if (this.currentSearch) {
      this.pRoulService
        ...
        .subscribe(
          (res: HttpResponse<IPRoul[]>) => this.onSuccess(res.body, res.headers, pageToLoad, !dontNavigate),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInPRouls(): void {
    this.eventSubscriber = this.eventManager.subscribe('pRoulListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IPRoul[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    this.ngbPaginationPage = this.page;
    if (navigate) {
      this.router.navigate(['/p-roul'], {
        queryParams: {
          page: this.page,
          size: this.itemsPerPage,
